...
{\asl} language --- \NEW{called {\aser}\footnote{The ER suffix stands for ``Encapsulated Reactivity''}} --- featuring \emph{plan encapsulation}, i.e. the possibility to define plans that fully
...
\end{itemize}
%
\NEW{A first prototype implementation of the new language has been developed
on top of Jason and ASTRA}.
%
...
%
We then introduce {\aser}, defining the main concepts,
syntax and informal semantics (Section~\ref{sec:proposal}). %--- first informally (Section~\ref{sec:proposal})
%and then providing the formalisation of some key aspect (Section~\ref{sec:formalisation}).
%
A  case study about the implementation of the Minority Game is
then used to discuss the approach (Section~\ref{sec:evaluation}).
%
Finally we conclude the paper discussing related work
